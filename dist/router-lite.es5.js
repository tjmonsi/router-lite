...
var RouterLite = (function (superclass) {
  function RouterLite () {
  ...
  if ( superclass ) RouterLite.__proto__ = superclass;
  RouterLite.prototype = Object.create( superclass && superclass.prototype );
  RouterLite.prototype.constructor = RouterLite;

  var prototypeAccessors = { currentRoute: { configurable: true },fallbackRoute: { configurable: true },path: { configurable: true } };
  var staticAccessors = { is: { configurable: true },observedAttributes: { configurable: true } };
  ...
  RouterLite.prototype.connectedCallback = function connectedCallback () {
    var this$1 = this;
    ...
    // element upgraded
    if (this.children) {
      for (var child of Array.from(this$1.children)) {
        this$1._getRoute(child);
      }
    }
  ...
  RouterLite.prototype.disconnectedCallback = function disconnectedCallback () {
    if (this._observer) { this._observer.disconnect(); }
  };

  prototypeAccessors.currentRoute.set = function (currentRoute) {
  ...
  prototypeAccessors.fallbackRoute.set = function (fallbackRoute) {
    this.__data.fallbackRoute = fallbackRoute;
    this.setAttribute('fallback-route', fallbackRoute);
  };

  prototypeAccessors.fallbackRoute.get = function () {
  ...
  RouterLite.prototype._childrenChanged = function _childrenChanged (changes) {
    var this$1 = this;

    for (var change of changes) {
      if (change.type === 'childList' && change.addedNodes) {
        for (var child of Array.from(change.addedNodes)) {
          this$1._getRoute(child);
        }
      }
    }
  ...
  RouterLite.prototype._getRoute = function _getRoute (child) {
  ...
  RouterLite.prototype._pathChanged = function _pathChanged (path) {
    var this$1 = this;

    var exec = null;
    var re = null;
    var keys = [];
    for (var route of this$1.routes) {
      keys = [];
      re = pathToRegexp(route, keys);
      exec = re.exec(path);
      if (exec) { return this$1._routeMatched(route, exec, keys); }
    }
    return this._routeMatched(this.fallbackRoute, [], []);
  };

  RouterLite.prototype._routeMatched = function _routeMatched (route, exec, keys) {
    var this$1 = this;
    ...
    Promise.resolve().then(function () {
      this$1.dispatchEvent(new window.CustomEvent('route-param-object-change', { detail: this$1.routeParamObject }));
      this$1.dispatchEvent(new window.CustomEvent('current-route-change', { detail: this$1.currentRoute }));
    });
  };

  RouterLite.prototype.attributeChangedCallback = function attributeChangedCallback (name, oldValue, newValue) {
    if (name === 'fallback-route' && this.__data.fallbackRoute !== newValue) {
      this.__data.fallbackRoute = newValue;
    }
  };

  Object.defineProperties( RouterLite.prototype, prototypeAccessors );
  Object.defineProperties( RouterLite, staticAccessors );
